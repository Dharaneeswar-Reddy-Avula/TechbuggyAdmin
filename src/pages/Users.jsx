      ...
      <div className="flex justify-end mt-[-80px]">
        <div className="relative w-full sm:w-64 mb-3">
          <input
            type="text"
            value={search}
            className="w-full rounded-lg border border-gray-300 dark:border-gray-600 px-4 py-2 pl-10 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition dark:bg-gray-800 dark:text-white"
            placeholder="Search Users..."
            onChange={(e) => setSearch(e.target.value)}
          />
          <CiSearch className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 dark:text-gray-300 text-xl" />
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800  rounded-xl shadow-md overflow-hidden">
        ...
        {loading && showSpinner && (
          <div className="flex flex-col items-center justify-center gap-4 py-8">
            <div className="w-16 h-16 border-4 border-blue-500 border-dashed rounded-full animate-spin"></div>
            <p className="text-blue-600 text-sm font-medium animate-pulse">Loading users, please wait...</p>
          </div>
        )}

        {error && (
          <div className="p-6 bg-red-50 dark:bg-red-950 text-red-600 dark:text-red-300 rounded-md mx-6 my-4 flex justify-between items-center">
            <span>{error}</span>
            <button
              onClick={fetchUsers}
              className="px-4 py-2 bg-red-100 dark:bg-red-900 hover:bg-red-200 dark:hover:bg-red-800 text-red-700 dark:text-red-300 rounded-md transition-colors"
            >
              Retry
            </button>
          </div>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-40">
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-6 w-full max-w-md relative">
              <div className="flex justify-between items-center mb-4">
                ...
                <button
                  onClick={() => setVisible(false)}
                  className="text-gray-500 hover:text-gray-700 dark:hover:text-white p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition"
                >
                  <span className="text-2xl font-bold leading-none">×</span>
                </button>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  ...
                  <input
                    ...
                    value={formData.username}
                    onChange={handleInputChange}
                    className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-800 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition dark:text-white"
                    placeholder="Enter username"
                  />
                </div>
                <div>
            ...
            <table className="min-w-full divide-y divide-gray-200  dark:divide-gray-400">
              <thead className="bg-gray-50 dark:bg-gray-700 ">
                <tr>
                  {["S.no", "Name", "Email", "Status", "Registered On", "Actions"].map((head, idx) => (
                    <th key={idx} className="px-6 py-3 text-left text-sm font-medium text-gray-500 dark:text-gray-300 uppercase">
                      {head}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                {filteredUsers.map((user, index) => (
                  <tr
                    key={user._id || index}
                    className="hover:bg-gray-50 dark:hover:bg-gray-700 dark:bg-gray-800 transition-colors"
                  >
                    <td className="px-6 py-4">{index + 1}</td>
                    <td className="px-6 py-4">{user.username}</td>
                    <td className="px-6 py-4">{user.email}</td>
                    <td className="px-6 py-4">
                      <span className="px-2 inline-flex text-sm font-semibold rounded-full bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-300">
                        Active
                      </span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end space-x-3">
                        <button
                          className="text-green-600 hover:text-green-900 p-1 rounded-md hover:bg-green-50 dark:hover:bg-green-500 transition-all"
                          title="View"
                          onClick={() => handleView(user)}
                        >
                          <AiOutlineEye className="text-xl" />
                        ...
                        <button
                          className="text-blue-600 hover:text-blue-900 p-1 rounded-md hover:bg-blue-50 dark:hover:bg-blue-500 transition-all"
                          title="Edit"
                          onClick={() => handleEdit(user)}
                        >
                          <MdEdit className="text-xl" />
                        ...
                        <button
                          className="text-red-600 hover:text-red-900 p-1 rounded-md hover:bg-red-50 dark:hover:bg-red-500 transition-all"
                          title="Delete"
                          onClick={() => handleDelete(user._id)}
                        >
                          <ImBin className="text-lg" />
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm transition-all">
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-6 w-full max-w-md relative animate-fade-in-up">
              <div className="flex justify-between items-center border-b border-gray-200 dark:border-gray-700 pb-3 mb-4">
                ...
                <button
                  onClick={() => setViewModalOpen(false)}
                  className="text-gray-500 hover:text-gray-700 dark:hover:text-white p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition"
                >
                  <IoCloseSharp className="text-2xl" />
                </button>
              </div>
              ...
              <div className="space-y-4 text-sm text-gray-700 dark:text-gray-300">
                <div className="flex flex-col">
                  <span className="text-xs text-gray-400">Username</span>
                  <span className="font-medium text-base dark:text-white">{viewUser.username}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-xs text-gray-400">Email</span>
                  <span className="font-medium text-base dark:text-white">{viewUser.email}</span>
                </div>
              </div>
        ...
        {confirmDeleteModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
            <div className="bg-white dark:bg-gray-900 p-6 rounded-xl shadow-xl w-full max-w-md">
              <h2 className="text-lg font-semibold mb-4 text-gray-800 dark:text-white">Delete User</h2>
              <p className="text-sm mb-2 text-gray-600 dark:text-gray-300">Please enter the reason for deleting this user:</p>
              <textarea
                rows={3}
                value={deleteReason}
                onChange={(e) => setDeleteReason(e.target.value)}
                placeholder="Reason for deletion..."
                className="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-800 text-gray-800 dark:text-white rounded-md p-2 mb-4 focus:outline-none focus:ring focus:border-blue-500"
              />
              <div className="flex justify-end space-x-2">
                <button
                ...
                <button
                  className={`px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 flex items-center justify-center ${deleting ? "cursor-not-allowed opacity-70" : ""
                    }`}
                  onClick={confirmDelete}
                  disabled={!deleteReason.trim() || deleting}
                >

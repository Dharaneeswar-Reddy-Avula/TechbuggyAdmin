...
const handleDelete = async (questionIndex) => {
  try {
    const response = await axios.delete(
      `http://localhost:8009/api/tests/question/${quizId}`,
      {
        params: { deleteIndex: questionIndex },
        headers: {
    ...
    // Optional: You can check response.data.success if your backend sends that
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
        ...
        {questions.map((q, qIndex) => (
          <div key={qIndex} className="flex items-center justify-between bg-white shadow-md p-4 rounded-lg mb-3">
            ...
            <div className="flex gap-2">
              <button className="text-blue-500" onClick={() => handleEdit(qIndex)}>
                <FaEdit />
              </button>
              <button className="text-red-500" onClick={() => handleDelete(qIndex)}>
                <FaTrash />
              </button>
            </div>
          </div>
      ...
      <div className="flex justify-end gap-4 mt-6">
        <button className="bg-blue-600 text-white px-4 py-2 rounded-md" onClick={handleNextQuestion}>
          Next Question
        </button>
